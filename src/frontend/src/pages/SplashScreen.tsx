...
import { Loader2, Monitor, Upload } from "lucide-react";
...
export default function SplashPage() {
  ...
  const fileInputId = "splash-file-input";
  const fileRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    const r = ref(db, "/settings/splashScreen");
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const path = `settings/splash_${Date.now()}`;
      const sRef = storageRef(storage, path);
      const snap = await uploadBytes(sRef, selectedFile);
      const url = await getDownloadURL(snap.ref);
      await set(ref(db, "/settings/splashScreen"), url);
  ...
  return (
    <div data-ocid="splash.page">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Splash Screen</h1>
        <p className="text-muted-foreground text-sm mt-0.5">
          Control the user app loading screen image
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground gap-2">
              ...
              <p className="text-sm">No splash screen set</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          >
            {preview ? (
              ...
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <p className="text-sm">Click to select splash image</p>
              </div>
            )}
          </label>
          <input
          ...
          {selectedFile && (
            <button
              type="button"
              data-ocid="splash.upload_button"
            ...
            >
              ...
              {uploading ? "Uploading..." : "Update Splash Screen"}
            </button>
          )}
        </div>
      </div>
